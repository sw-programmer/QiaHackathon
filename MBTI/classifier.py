...
class MLPClassifier(nn.Module):
    ...
    def __init__(self,
        ...
        emb_dim = 2
        self.embedding = nn.Embedding(num_embeddings=61, embedding_dim=emb_dim, padding_idx=0)
        self.dimensions = [768 + emb_dim + 5] + hidden_dim + [self.num_classes]        # dimension list for all layers 
        # layer stacks
        self.layers = nn.ModuleList(
            [nn.Linear(self.dimensions[i - 1], self.dimensions[i]) for i in range(1, len(self.dimensions))])
        self.bn = nn.ModuleList(
            [nn.BatchNorm1d(i) for i in hidden_dim]
        )
